...
`dgpsi` provides a flexible toolbox for Gaussian process (GP), deep Gaussian process (DGP) and linked (D)GP emulations. In this guide we give a quick demonstration of the package on modeling the popular motorcycle dataset [@silverman1985]. There are other examples showing the functionality of the package in `Articles`, and how to customize DGP structures. A comprehensive reference of available functions is documented in `Reference`. 
...
```{r}
m <- dgp(X, Y, depth = 3, likelihood = "Hetero", verb = 2)
```
...
## |  Layer 2  |  Node 1  |  GP (Squared-Exp)   |     [1.000]     |     1.000     | 1.000e-06 (fixed) |    [1]     |        [1]        |
## |  Layer 2  |  Node 2  |  GP (Squared-Exp)   |     [1.000]     |     1.000     | 1.000e-06 (fixed) |    [1]     |        [1]        |
...
## Iteration 500: Layer 3: 100%|██████████| 500/500 [00:19<00:00, 26.00it/s]
```

We choose a heteroskedastic Gaussian likelihood by setting `likelihood = "Hetero"` since the data drawn in the plot show varying noises. We set `verb = 2` so [dgp()] prints out a table that summarizes the generated DGP structure and we can check if everything is correct before proceeding to training.
...
## | Layer No. | Node No. |        Type         | Length-scale(s) |   Variance    |      Nugget       | Input Dims | Global Connection |
## +-----------+----------+---------------------+-----------------+---------------+-------------------+------------+-------------------+
## |  Layer 1  |  Node 1  |  GP (Squared-Exp)   |     [1.251]     | 1.000 (fixed) | 1.000e-06 (fixed) |    [1]     |        No         |
## |  Layer 2  |  Node 1  |  GP (Squared-Exp)   |     [0.434]     |     1.796     | 1.000e-06 (fixed) |    [1]     |        [1]        |
## |  Layer 2  |  Node 2  |  GP (Squared-Exp)   |     [1.951]     |    29.327     | 1.000e-06 (fixed) |    [1]     |        [1]        |
## |  Layer 3  |  Node 1  | Likelihood (Hetero) |       NA        |      NA       |        NA         |   [1, 2]   |        NA         |
## +-----------+----------+---------------------+-----------------+---------------+-------------------+------------+-------------------+
## 1. 'Input Dims' presents the indices of GP nodes in the feeding layer whose outputs are used as the input to the current GP.
## 2. 'Global Connection' indicates the dimensions (i.e., column numbers) of the global input data that are used as additional input dimensions to the current GP.
