...
We now make predictions from the trained GP emulator to evaluate the emulation performance for Model 1 over $[0,1]$:
...
## Iteration 500: Layer 2: 100%|██████████| 500/500 [00:02<00:00, 236.81it/s]
```
...
The following plot visualizes the emulation performance of the trained DGP emulator `m2`:
...
Note that each layer of an emulator system is represented by a list. In our case, there is one GP emulator in the first layer and a DGP emulator in the second layer. Therefore, we need to put the corresponding emulators in a list using `list()`.
...
```{r}
# linked emulator
m_link <- predict(m_link, x = test_x1)
# GP emulator
