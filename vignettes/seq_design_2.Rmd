...
## Iteration 500: Layer 2: 100%|██████████| 500/500 [00:02<00:00, 229.49it/s]
## Imputing ... done
```
...
## Initializing the DGP emulator ... done
## Training the DGP emulator: 
## Iteration 500: Layer 2: 100%|██████████| 500/500 [00:01<00:00, 310.61it/s]
## Imputing ... done
```
...
## Initializing the DGP emulator ... done
## Training the DGP emulator: 
## Iteration 500: Layer 2: 100%|██████████| 500/500 [00:01<00:00, 395.78it/s]
## Imputing ... done
```
...
```
## Initializing ... done
##  * RMSE: 0.361635 0.129256 0.029830
## Iteration 1:
##  - Locating ... done
##  * Next design point (Emulator1): 0.995697
##  * Next design point (Emulator2): 0.003305
##  * Next design point (Emulator3): 0.003305
##  - Updating and re-fitting ... done
##  - Validating ... done
##  * RMSE: 0.361063 0.144323 0.008922
## Iteration 2:
##  - Locating ... done
##  * Next design point (Emulator1): 0.002217
##  * Next design point (Emulator2): 0.997442
##  * Next design point (Emulator3): None (target reached)
##  - Updating and re-fitting ... done
##  - Validating ... done
##  * RMSE: 0.406302 0.129734 0.008922
##  
##  ...
##  
## Iteration 10:
##  - Locating ... done
##  * Next design point (Emulator1): 0.297411
##  * Next design point (Emulator2): 0.597249
##  * Next design point (Emulator3): None (target reached)
##  - Updating and re-fitting ... done
##  - Validating ... done
##  * RMSE: 0.121305 0.019911 0.008922
## Targets are not reached for all emulators at the end of the sequential design.
```

It can be seen that at the first step, the DGP emulator for the third output has already reached the target, so for the rest of the steps no further refinements (i.e., additions of design points to the third DGP emulator) are performed. At the end of the first wave, the DGP emulators for both the first and the second outputs have not reached the target yet. At this point, we can proceed to a second wave by repeating the command above, but we show below an alternative way, in which we define an aggregation function that aggregates criterion scores across the three outputs such that the same design points are added to the three emulators at each step (instead of different design points for each emulator). We define the aggregation function `g` that aggregate scores by calculating their weighted average: 
...
Since the third emulator has already reached the target, we assign zero weight to it. As the second emulator is much closer to the target than the first one after the first wave, we assign a larger weight to the first emulator:

```{r}
weight <- c(0.8, 0.2, 0)
```

We now pass both the aggregate function `g()` and its `weight` argument to `design()` for the second wave of the sequential design with a further 10 steps:

```{r}
# 2nd wave with 10 steps
m <- design(m, N = 10, limits = lim, f = f, x_test = validate_x, y_test = validate_y, aggregate = g, target = 0.01, weight = weight)
```
```
##  Initializing ... done
##   * RMSE: 0.121305 0.019911 0.008922
##  Iteration 1:
##   - Locating ... done
##   * Next design point: 0.385054
##   - Updating and re-fitting ... done
##   - Validating ... done
##   * RMSE: 0.102591 0.018566 0.008922
##  Iteration 2:
##   - Locating ... done
##   * Next design point: 0.155967
##   - Updating and re-fitting ... done
##   - Validating ... done
##   * RMSE: 0.079752 0.015872 0.008922
##  Iteration 3:
##   - Locating ... done
##   * Next design point: 0.024872
##   - Updating and re-fitting ... done
##   - Validating ... done
##   * RMSE: 0.049682 0.009417 0.008922
##  
##  ...
##  
##  Iteration 10:
##   - Locating ... done
##   * Next design point: 0.037399
##   - Updating and re-fitting ... done
##   - Validating ... done
##   * RMSE: 0.007271 0.009417 0.008922
##  Target reached! The sequential design stops at step 10.
```

The second emulator reached the target at step 3 and then no further refinements of it are performed. The first emulator finally reached the target at the final step of the second wave. The sequential designs of the three emulators can be plotted with `draw()`:
...
rmse_static_1 <- data.frame('N' = c(10, 20, 30), 'rmse' = c(m11$oos$rmse, m21$oos$rmse, m31$oos$rmse), 'design' = c('lhd-10', 'lhd-20', 'lhd-30'))
...
rmse_static_2 <- data.frame('N' = c(10, 20, 30), 'rmse' = c(m12$oos$rmse, m22$oos$rmse, m32$oos$rmse), 'design' = c('lhd-10', 'lhd-20', 'lhd-30'))
...
rmse_static_3 <- data.frame('N' = c(10, 20, 30), 'rmse' = c(m13$oos$rmse, m23$oos$rmse, m33$oos$rmse), 'design' = c('lhd-10', 'lhd-20', 'lhd-30'))
```

and add them to the sequential design validation plot (in log-scale) for comparisons:

```{r}
p1 <- draw(mm, emulator = 1, type = 'rmse', log = T) + 
  geom_point(data = rmse_static_1, mapping = aes(x = N, y = rmse, group = design, shape = design), color = '#E69F00', size = 1.5) +
  scale_shape_manual(values = c(3, 4, 8))
p2 <- draw(mm, emulator = 2, type = 'rmse', log = T) + 
  geom_point(data = rmse_static_2, mapping = aes(x = N, y = rmse, group = design, shape = design), color = '#E69F00', size = 1.5) +
  scale_shape_manual(values = c(3, 4, 8))
p3 <- draw(mm, emulator = 3, type = 'rmse', log = T) + 
  geom_point(data = rmse_static_3, mapping = aes(x = N, y = rmse, group = design, shape = design), color = '#E69F00', size = 1.5) +
  scale_shape_manual(values = c(3, 4, 8))
