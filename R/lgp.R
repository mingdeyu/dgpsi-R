...
#'     in the corresponding emulator object (produced by [gp()] or [dgp()]) within `emulators` (the next argument following `struc`). The `id` slot
...
#'     corresponding emulator object. The `id` slot is generated or set as described above for `From_Emulator`.
#'   * `From_Output`: a single integer specifying the output dimension from the `From_Emulator` that is being connected to the
#'     input dimension of the `To_Emulator`.
#'   * `To_Input`: a single integer specifying the input dimension of the `To_Emulator` that is receiving the output dimension
...
#'
#'   Additionally, the special value `"Global"` in `From_Emulator` can be used to represent global input data, linking a
#'   specified dimension of the global input directly to an input dimension of an emulator.
#'
#'   **Note:** When using this data frame option, `emulators` argument must be provided.
