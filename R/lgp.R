...
#'     in the corresponding emulator object (produced by [gp()] or [dgp()]) within `emulators` (the next argument following `struc`), or it should be special value `"Global"`, indicating the global inputs to the model chain or network. The `id` slot
...
#'     input dimension of the `To_Emulator` specified by `To_Input`. If `From_Emulator` is the `"Global"` value, then `From_Output` indicates the dimension of the global input passed to the `To_Emulator`.
...
#'     If the system consists of only GP emulators, `B` is set to `1` automatically. Defaults to `10`.
