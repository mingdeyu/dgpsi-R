  ...
  if ( batch_size==1 ){
    ...
    for ( i in 1:n_emulators ){
      ...
      if ( inherits(obj_i,"gp") ){
        res = obj_i$emulator_obj$metric(x_cand = if (is.list(x_cand)) {x_cand[[i]]} else {x_cand}, method = 'ALM', score_only = TRUE)
        scores[[i]] <- res
      } else {
        ...
        if ( identical(workers,as.integer(1)) ){
          ...
          res = obj_i$emulator_obj$pmetric(x_cand = if (is.list(x_cand)) {x_cand[[i]]} else {x_cand}, method = 'ALM', score_only = TRUE, core_num = workers)
        }
    ...
    for (i in 1:batch_size){
      ...
      for ( j in 1:n_emulators ){
        ...
        if ( inherits(obj_j,"gp") ){
          res = emulator_obj_list[[j]]$metric(x_cand = if (is.list(x_cand)) {x_cand[[j]][idx_x_cand[[j]],,drop=F]} else {x_cand[idx_x_cand[[j]],,drop=F]}, method = 'ALM', score_only = TRUE)
          scores[[j]] <- res
        } else {
          ...
          if ( identical(workers,as.integer(1)) ){
            ...
            res = emulator_obj_list[[j]]$pmetric(x_cand = if (is.list(x_cand)) {x_cand[[j]][idx_x_cand[[j]],,drop=F]} else {x_cand[idx_x_cand[[j]],,drop=F]}, method = 'ALM', score_only = TRUE, core_num = workers)
          }
