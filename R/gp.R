...
#' * `2`: trace information on GP initialization and a summary table of the initialized GP emulator.
#' * `1`: all information as in `2` except for the summary table.
#' * `0`: no trace information and the summary table.
#'
#' When `verb = 2`, you will have a chance to check the specified GP emulator (especially when a customized `struc` is provided) and decide if you want to proceed to training. Defaults to `1`.
...
gp <- function(X, Y, struc = NULL, name = 'sexp', lengthscale = rep(0.2, ncol(X)), nugget_est = FALSE, nugget = 1e-6, verb = 1, internal_input_idx = NULL, linked_idx = NULL) {
  ...
  if ( verb == 2|verb == 1 ) message("Initializing the GP emulator ...", appendLF = FALSE)
  ...
  if ( verb == 2 ){
    ...
    message(" done")
    Sys.sleep(0.5)
    message("Summarizing the initialized GP emulator ...", appendLF = FALSE)
