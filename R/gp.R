...
#'     Defaults to `TRUE`.
#' @param check_rep `r new_badge("new")` a bool indicating whether to check for repetitions in the dataset, i.e., if one input
...
#' @md
#' @export
gp <- function(X, Y, name = 'sexp', lengthscale = rep(0.1, ncol(X)), bounds = NULL, prior = 'ref', nugget_est = FALSE, nugget = ifelse(nugget_est, 0.01, 1e-8), scale_est = TRUE, scale = 1., training = TRUE, verb = TRUE, check_rep = TRUE, vecchia = FALSE, M = 25, ord = NULL, id = NULL) {
  ...
  res[['container_obj']] <- pkg.env$dgpsi$container(obj$export(), NULL)
