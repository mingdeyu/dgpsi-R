...
#' @method pei gp
#' @export
pei.gp <- function(object, x_cand, pseudo_points = NULL, batch_size = 1, M = 50, workers = 1, ...) {
  ...
  #set gp params
  default_param <- list(name = 'matern2.5', nugget = 1e-12, verb = FALSE, vecchia = object[['specs']][['vecchia']], M = object[['specs']][['M']])
  add_arg <- list(...)
  ...
  score <- log(object$emulator_obj$esloo(m = M))
  score_gp <- do.call(gp, c(list(training_input, score), final_gp_arg))
  score_gp <- predict.gp(score_gp, x_cand, M = M, cores = workers)
  ...
  for ( i in 1:batch_size){
    if (score_gp[['specs']][['vecchia']]){
      NNarray = pkg.env$dgpsi$vecchia$get_pred_nn(t(t(x_cand)/c(score_gp$emulator_obj$kernel$length)), t(t(pseudo_training_points)/c(score_gp$emulator_obj$kernel$length)), M)
      rf <- rep(0, nrow(x_cand))
      for (j in 1:nrow(x_cand)){
        idx_NN <- NNarray[j,]
        idx_NN <- idx_NN[idx_NN>=0] + 1
        sub_pseudo_training_points <- pseudo_training_points[idx_NN,]
        rf[j] <- prod(1 - pkg.env$dgpsi$vecchia$K_vec_nb(sub_pseudo_training_points,
                      reticulate::np_array(x_cand[j,]), reticulate::np_array(score_gp$emulator_obj$kernel$length), score_gp$emulator_obj$kernel$name))
      }
    } else {
...
#' @method pei dgp
#' @export
pei.dgp <- function(object, x_cand, pseudo_points = NULL, batch_size = 1, M = 50, workers = 1, aggregate = NULL, ...) {
  if ( is.null(pkg.env$dgpsi) ) {
    init_py(verb = F)
    if (pkg.env$restart) return(invisible(NULL))
  }
  ...
  if ( identical(workers,as.integer(1)) ){
    score <- log(object$emulator_obj$esloo(training_input, training_output, M))
  } else {
    score <- log(object$emulator_obj$pesloo(training_input, training_output, M, core_num = workers))
  }
  for ( i in 1:n_dim_Y ){
  ...
  for ( i in 1:batch_size){
    ...
    for ( j in 1:n_dim_Y ){
      if (object[['specs']][['vecchia']]){
        NNarray = pkg.env$dgpsi$vecchia$get_pred_nn(t(t(x_cand)/c(length_list[[j]])), t(t(pseudo_training_points)/c(length_list[[j]])), M)
        rf <- rep(0, nrow(x_cand))
        for (k in 1:nrow(x_cand)){
          idx_NN <- NNarray[k,]
          idx_NN <- idx_NN[idx_NN>=0] + 1
          sub_pseudo_training_points <- pseudo_training_points[idx_NN,]
          rf[k] <- prod(1 - pkg.env$dgpsi$vecchia$K_vec_nb(sub_pseudo_training_points,
                      reticulate::np_array(x_cand[k,]), reticulate::np_array(length_list[[j]]), name_list[[j]]))
        }
      } else {
        rf <- pkg.env$np$prod(1 - pkg.env$dgpsi$functions$k_one_vec(pseudo_training_points,
...
#' @method pei bundle
#' @export
pei.bundle <- function(object, x_cand, pseudo_points = NULL, batch_size = 1, M = 50, workers = 1, aggregate = NULL, ...) {
  if ( is.null(pkg.env$dgpsi) ) {
    init_py(verb = F)
    if (pkg.env$restart) return(invisible(NULL))
  }
  ...
  for ( i in 1:n_emulators ){
    ...
    #set gp params
    default_param <- list(name = 'matern2.5', nugget = 1e-12, verb = FALSE, vecchia = obj_i[['specs']][['vecchia']], M = obj_i[['specs']][['M']])
    final_gp_arg <- utils::modifyList(default_param, gpparam)
    if ( inherits(obj_i,"gp") ){
      score <- log(obj_i$emulator_obj$esloo(m = M))
    } else {
      ...
      if ( identical(workers,as.integer(1)) ){
        score <- log(obj_i$emulator_obj$esloo(training_input[[i]], training_output[[i]], M))
      } else {
        score <- log(obj_i$emulator_obj$pesloo(training_input[[i]], training_output[[i]], M, core_num = workers))
      }
    }
    score_gp <- do.call(gp, c(list(training_input[[i]], score), final_gp_arg))
    length_list[[i]] <- score_gp$emulator_obj$kernel$length
    name_list[[i]] <- score_gp$emulator_obj$kernel$name
    score_gp <- predict.gp(score_gp, if (is.list(x_cand)) {x_cand[[i]]} else {x_cand}, M = M, cores = workers)
    score_mean <- score_gp$results$mean
    score_var <- score_gp$results$var
    score_std <- sqrt(score_gp$results$var)
    score_scale <- as.numeric(score_gp$emulator_obj$kernel$scale)
  ...
  for ( i in 1:batch_size){
    ...
    for ( j in 1:n_emulators ){
      if (object[[paste('emulator',j,sep='')]][['specs']][['vecchia']]){
        x_cand_j <- if (is.list(x_cand)) {t(t(x_cand[[j]])/c(length_list[[j]]))} else {t(t(x_cand)/c(length_list[[j]]))}
        NNarray = pkg.env$dgpsi$vecchia$get_pred_nn(x_cand_j, t(t(pseudo_training_points[[j]])/c(length_list[[j]])), M)
        rf <- rep(0, nrow(x_cand_j))
        for (k in 1:nrow(x_cand_j)){
          idx_NN <- NNarray[k,]
          idx_NN <- idx_NN[idx_NN>=0] + 1
          sub_pseudo_training_points <- pseudo_training_points[[j]][idx_NN,]
          rf[k] <- prod(1 - pkg.env$dgpsi$vecchia$K_vec_nb(sub_pseudo_training_points,
                    if (is.list(x_cand)) {reticulate::np_array(x_cand[[j]][k,])} else {reticulate::np_array(x_cand[k,])}, reticulate::np_array(length_list[[j]]), name_list[[j]]))
        }
      } else {
