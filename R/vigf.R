...
#' @method vigf gp
#' @export
vigf.gp <- function(object, x_cand, batch_size = 1, M = 50, workers = 1, ...) {
...
#' @method vigf dgp
#' @export
vigf.dgp <- function(object, x_cand, batch_size = 1, M = 50, workers = 1, aggregate = NULL, ...) {
  if ( is.null(pkg.env$dgpsi) ) {
    init_py(verb = F)
    if (pkg.env$restart) return(invisible(NULL))
  }
...
#' @method vigf bundle
#' @export
vigf.bundle <- function(object, x_cand, batch_size = 1, M = 50, workers = 1, aggregate = NULL, ...) {
  if ( is.null(pkg.env$dgpsi) ) {
    init_py(verb = F)
    if (pkg.env$restart) return(invisible(NULL))
  }
