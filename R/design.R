...
#'     `aggregate`.
#' * `f` should be defined as a function with only one argument which is a vector giving values of different simulator input dimensions. `f`
#'   can return a single or two outputs.
#'   - if `f` has one output, it should return a vector that gives values of different simulator output dimensions.
#'   - if `f` has two outputs, it should return a list with two slots:
#'     - the first slot is a vector that gives values of different simulator output dimensions.
...
#'   - the positions of input dimensions of `f` are consistent with those of the emulator;
#'   - the positions of output dimensions of `f` are consistent with those of the emulator, or the order of emulators placed in `object` if `object` is an
...
#' @name design
#' @export
design <- function(object, N, x_cand, y_cand, n_cand, limits, f, freq, x_test, y_test, method, nugget_s, verb, check_point, cores, ...){
  UseMethod("design")
}
...
#' @method design gp
#' @export
design.gp <- function(object, N, x_cand = NULL, y_cand = NULL, n_cand = 200, limits = NULL, f = NULL, freq = c(1, 1), x_test = NULL, y_test = NULL, method = 'ALM', nugget_s = 1., verb = TRUE, check_point = NULL, cores = c(1, 1), ...) {
  ...
  if ( is.null(x_cand) ) {
    ...
    for ( i in 1:N ){
      ...
      res <- locate(object, n_cand = n_cand, limits = limits, method = method, nugget_s = nugget_s, verb = FALSE, cores = cores[1])
      if ( verb ) {
        message(" done")
        message(paste(c(" * Next design point:", sprintf("%.06f", res$location)), collapse=" "))
      }

      X <- rbind(X, res$location)
      Y <- rbind(Y, f(res$location))
    ...
    for ( i in 1:N ){
      ...
      res <- locate(object, x_cand = sub_cand, method = method, nugget_s = nugget_s, verb = FALSE, cores = cores[1])
      if ( verb ) {
        message(" done")
        message(paste(c(" * Next design point:", sprintf("%.06f", res$location)), collapse=" "))
      }
      ...
      X <- rbind(X, res$location)
      Y <- rbind(Y, y_cand[idx_sub_cand,,drop=FALSE][res$index,])
      idx_x_acq <- c(idx_x_acq, idx_sub_cand[res$index])
...
#' @method design dgp
#' @export
design.dgp <- function(object, N, x_cand = NULL, y_cand = NULL, n_cand = 200, limits = NULL, f = NULL, freq = c(1, 1), x_test = NULL, y_test = NULL, method = 'ALM', nugget_s = 1., verb = TRUE, check_point = NULL, cores = c(1, 1), threading = FALSE, train_N = 100, aggregate = NULL, ...) {
  ...
  if ( is.null(x_cand) ) {
    ...
    for ( i in 1:N ){
      ...
      res <- locate(object, n_cand = n_cand, limits = limits, method = method, nugget_s = nugget_s, verb = FALSE, cores = cores[1], threading = threading, aggregate = g)
      if ( verb ) {
        message(" done")
    ...
    for ( i in 1:N ){
      ...
      res <- locate(object, x_cand = sub_cand, method = method, nugget_s = nugget_s, verb = FALSE, cores = cores[1], threading = threading, aggregate = g)
      if ( verb ) {
        message(" done")
        if ( nrow(res$location)==1 ){
          message(paste(c(" * Next design point:", sprintf("%.06f", res$location[1,])), collapse=" "))
      ...
      X <- rbind(X, res$location)
      Y <- rbind(Y, y_cand[idx_sub_cand,,drop=FALSE][res$index,,drop=FALSE])
      idx_x_acq <- c(idx_x_acq, idx_sub_cand[res$index])
      idx_x_cand <- idx_x_cand0[-idx_x_acq]
...
#' @export
design.bundle <- function(object, N, x_cand = NULL, y_cand = NULL, n_cand = 200, limits = NULL, f = NULL, freq = c(1, 1), x_test = NULL, y_test = NULL, method = 'ALM', nugget_s = 1., verb = TRUE, check_point = NULL, cores = c(1, 1), threading = FALSE, train_N = 100, aggregate = NULL, ...) {
