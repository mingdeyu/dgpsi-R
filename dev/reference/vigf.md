...
using the Variance of Improvement for Global Fit (VIGF). For VIGF on GP
emulators, see the reference below.
...
- ...:
  ...
  `vigf()`) that are used by `aggregate` can be passed here.
...
- aggregate:
  ...
  an R function that aggregates scores of the VIGF across different
...
Mohammadi, H., & Challenor, P. (2022). Sequential adaptive design for
emulating costly computer codes. *arXiv:2206.12113*.
